...
class DashboardFragment: Fragment() {
    ...
    private fun fetchAvg(){
        lifecycleScope.launch(IO) {
            avgCalories.text = (activity?.application as FoodApplication).db.ActivityDao().avg().toString()
            minCal.text=(activity?.application as FoodApplication).db.ActivityDao().min().toString()
            maxCal.text=(activity?.application as FoodApplication).db.ActivityDao().max().toString()
        }
    }
